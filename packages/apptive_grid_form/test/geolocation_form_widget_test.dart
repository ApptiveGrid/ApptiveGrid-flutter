...
void main() {
  ...
  group('Decoration', () {
    ...
    testWidgets('Prefilled Required Form is validated',
            (tester) async {
          final action = FormAction('actionUri', 'POST');
          ...
          final geolocationData = GeolocationDataEntity(const Geolocation(latitude: 47, longitude: 11));
          final mockGeolocationHttpClient = MockHttpClient();
          ...
          when(() => apptiveGridClient.performAction(action, any(), saveToPendingItems: any(named: 'saveToPendingItems'))).thenAnswer((_) async => Response('', 200));
          ...
          expect(find.text(requiredString), findsNothing);
          verify(() => apptiveGridClient.performAction(action, any(), saveToPendingItems: any(named: 'saveToPendingItems'))).called(1);
        });
  });
}
