...
void main() {
  setUpAll(() {
    ...
    registerFallbackValue(const CameraPosition(target: const LatLng(0, 0)));
  ...
  group('TextInput', () {
    ...
    testWidgets('Initial Location Geocodes to address', (tester) async {
      ...
      final target = TestApp(
        options: ApptiveGridOptions(formWidgetConfigurations: [
      ...
      when(() => mockGeolocationHttpClient.get(any(),
    ...
    testWidgets('Search with initial location uses location for results', (tester) async {
      final mockGeolocationHttpClient = MockHttpClient();
      final target = TestApp(
        options: ApptiveGridOptions(formWidgetConfigurations: [
          GeolocationFormWidgetConfiguration.withHttpClient(
      ...
      final capturedUri = verify(() => mockGeolocationHttpClient.get(captureAny<Uri>(that: predicate<Uri>((uri) => uri.path.contains('queryautocomplete'))),
          headers: any(named: 'headers'))).captured.first as Uri;

      expect(capturedUri.queryParameters['location'], '47.0,11.0');
    });

    testWidgets('Filled Textfield shows clear button, click clears button', (tester) async {
  ...
  group('My Location Button', () {
    ...
    testWidgets('Geocoding returns no results '
  ...
  group('Map', () {
    ...
    setUp(() {
      ...
      when(() => mockMap.init(any())).thenAnswer((invocation) async =>
      ...
      when(() => mockMap.updateTileOverlays(
          newTileOverlays: any(named: 'newTileOverlays'),
          mapId: any(named: 'mapId'))).thenAnswer((_) async {});
      ...
      when(() => mockGeolocationHttpClient.get(any(),
        ...
        if (requestUri.path.contains('geocode')) {
          ...
          if (requestUri.queryParameters['latlng'] == '47.0,11.0') {
            response = GeocodingResponse(status: 'OK', results: [
              GeocodingResult(
                  geometry: Geometry(location: Location(lat: 47, lng: 11)),
                  placeId: 'placeId',
                  formattedAddress: 'ZWEIDENKER')
  ...
  group('Decoration', () {
    testWidgets('Required shows hint and updates message if changed',
      ...
      await tester.scrollUntilVisible(
          find.byType(ActionButton, skipOffstage: false), 400,
      ...
      await tester.scrollUntilVisible(
          find.byIcon(Icons.my_location, skipOffstage: false), -400,
          scrollable: scrollable);
      await tester.pump();
      await tester.tap(
        find.byIcon(Icons.my_location),
