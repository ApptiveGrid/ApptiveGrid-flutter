...
void main() {
  ...
  final field =
      GridField(id: 'fieldId', name: 'name', type: DataType.user, links: {
  ...
  group('Submit Logic', () {
    testWidgets('Has Value, submits form', (tester) async {
      ...
      when(() => client.loadForm(uri: formUri))
    ...
    testWidgets('No Value but required, shows error', (tester) async {
      ...
      expect(find.text('name must not be empty', skipOffstage: false),
          findsOneWidget);
    });
    ...
    testWidgets('Updates Value submits new value', (tester) async {
      ...
      when(() => client.performApptiveLink<List<DataUser>>(
              link: field.links[ApptiveLinkType.collaborators]!,
              queryParameters: any(named: 'queryParameters'),
              parseResponse: any(named: 'parseResponse')))
          .thenAnswer((invocation) async {
        final parseResponse = invocation.namedArguments[Symbol('parseResponse')]
  ...
  group('Filter Collaborators', () {
    testWidgets('Filter Collaborators', (tester) async {
      ...
      when(() => client.performApptiveLink<List<DataUser>>(
        ...
        final parseResponse = invocation.namedArguments[Symbol('parseResponse')]
            as Future<List<DataUser>?> Function(Response);
        final query = (invocation.namedArguments[Symbol('queryParameters')]
      ...
      expect(
          find.descendant(
              of: find.byType(ListTile), matching: find.byType(DataUserWidget)),
          findsNWidgets(2));
      ...
      verify(() => client.performApptiveLink<List<DataUser>>(
          link: field.links[ApptiveLinkType.collaborators]!,
          queryParameters: {'matching': 'Da'},
          parseResponse: any(named: 'parseResponse'))).called(1);
      verify(() => client.performApptiveLink<List<DataUser>>(
          link: field.links[ApptiveLinkType.collaborators]!,
    ...
    testWidgets('No collaborators shows nothing', (tester) async {
      ...
      verifyNever(() => client.performApptiveLink<List<DataUser>>(
          link: any(named: 'link'),
          queryParameters: any(named: 'queryParameters'),
          parseResponse: any(named: 'parseResponse')));
    });
    ...
    testWidgets('Clear search value', (tester) async {
      ...
      verify(() =>
          client.performApptiveLink<List<DataUser>>(
              link: field.links[ApptiveLinkType.collaborators]!,
              queryParameters: {},
              parseResponse: any(named: 'parseResponse'))).called(
          3); // Once for initial load, once for focus of search field, once after cleared input
    });
  });
}
